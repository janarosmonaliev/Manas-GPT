"""
Sample from a trained model, taken from Andrej Karpatov's nanoGPT repository
"""
import os
import pickle
...
if init_from == 'resume':
    ...
    ckpt_path = os.path.join(out_dir, 'manas-gpt-v2.pt')
...
if load_meta:
    ...
    # encoding_name = tiktoken.encoding_for_model("gpt-4o")
...
with torch.no_grad():
    with ctx:
        for k in range(num_samples):
            ...
            open('manas_synthetic_textv2.txt', 'a').write(decode(y[0].tolist()).replace('<|endoftext|>', '\n') + '\n')
